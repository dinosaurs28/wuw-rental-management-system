  ...
  return {
    revenueTrends: {
      ...
      data: data.revenueTrends.map((item) => ({
        ...
        "Avg Revenue Per Booking (₹)":
          item.avgRevenuePerBooking?.toFixed(2) || 0,
    ...
    categoryBreakdown: {
      ...
      data: data.categoryBreakdown.map((item) => ({
        ...
        "Avg Revenue Per Vehicle (₹)":
          item.avgRevenuePerVehicle?.toFixed(2) || 0,
      })),
    },
    branchComparison: {
    ...
    paymentMethods: {
      ...
      data: data.paymentMethods.map((item) => ({
        "Payment Method":
          item.paymentMethod === "ONLINE_RAZORPAY"
            ? "Online"
            : item.paymentMethod,
        "Total Revenue (₹)": item.totalRevenue,
    ...
    combined: {
      ...
      data: [
        { Section: "Revenue Trends", "Data Points": data.revenueTrends.length },
        {
          ...
          "Data Points": data.categoryBreakdown.length,
        },
        {
          Section: "Branch Comparison",
          "Data Points": data.branchComparison.length,
        },
        {
          Section: "Payment Methods",
          "Data Points": data.paymentMethods.length,
        },
        {
          Section: "Total Revenue",
          "Data Points": `₹${data.branchComparison.reduce((sum, b) => sum + b.totalRevenue, 0).toLocaleString()}`,
        },
      ],
    },
  };
