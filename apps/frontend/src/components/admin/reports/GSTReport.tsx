...
import { DollarSign, FileText, TrendingUp, Building2, CreditCard } from 'lucide-react';
...
export const GSTReport = ({
    ...
    const columns: ColumnDef<Invoice>[] = [
        ...
        },
        {
            ...
            header: 'Taxable Amt',
            cell: ({ row }) => <span className="font-medium">{formatCurrency(row.original.taxableAmount)}</span>,
        },
        {
            accessorKey: 'totalGST',
            header: 'GST',
            cell: ({ row }) => <span className="font-medium text-orange-600">{formatCurrency(row.original.totalGST)}</span>,
        },
        {
        ...
        <div className="container mx-auto p-4 md:p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard
                    ...
                    icon={DollarSign}
                    variant="revenue"
                    trend={{ value: data ? data.summary.effectiveGSTRate : 0, label: 'effective rate' }}
                    isLoading={loading}
                />
                <MetricCard
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-green-700">CGST (Central)</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-green-600">
                            ₹{data.summary.totalCGST.toLocaleString('en-IN')}
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">9% on intra-state</p>
                    </CardContent>
                </Card>

                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-blue-700">SGST (State)</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-amber-700">IGST (Integrated)</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-amber-600">
                            ₹{data.summary.totalIGST.toLocaleString('en-IN')}
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">18% on inter-state</p>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <CardContent>
                        <ResponsiveContainer width="100%" height={300}>
                            <LineChart data={data.monthlyBreakdown}>
                                ...
                                <Tooltip formatter={(value: number) => `₹${value.toLocaleString('en-IN')}`} />
                                <Legend />
                                <Line type="monotone" dataKey="totalGST" stroke="#FF5F00" strokeWidth={2} name="Total GST" />
                                <Line type="monotone" dataKey="taxableAmount" stroke="#3b82f6" strokeWidth={2} name="Taxable Amount" />
                            </LineChart>
                        </ResponsiveContainer>
                    </CardContent>
                </Card>
