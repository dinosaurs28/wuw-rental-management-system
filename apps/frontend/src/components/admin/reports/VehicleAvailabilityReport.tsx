...
export const VehicleAvailabilityReport = () => {
    ...
    const [reportData, setReportData] = useState<AvailabilityReportData | null>(null);
    ...
    const fetchReportData = async () => {
        ...
        try {
            ...
            const result = await adminService.getVehicleAvailabilityReport(params as any);
            setReportData(result.data);
        } catch (error) {
            console.error('Error fetching report:', error);
            toast.error('Failed to load vehicle availability report. Please try again.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const categoryChartData = reportData?.categoryBreakdown.map((item) => ({
        name: item.category,
    ...
    const statusDistribution = [
        { name: 'Available', value: reportData?.summary.currentlyAvailable || 0, color: '#10B981' },
        { name: 'Rented', value: reportData?.summary.currentlyRented || 0, color: '#FF5F00' },
        { name: 'Maintenance', value: reportData?.summary.inMaintenance || 0, color: '#F59E0B' },
        { name: 'Inactive', value: reportData?.summary.inactive || 0, color: '#EF4444' },
    ];
                        ...
                        {categoryChartData.length > 0 ? (
                            <ResponsiveContainer width="100%" height={250}>
                                <BarChart data={categoryChartData}>
                                    ...
                                    <Bar dataKey="rented" stackId="a" fill="#FF5F00" name="Rented" />
