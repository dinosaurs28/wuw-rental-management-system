...
export const InsurancePermitExpiryReport = () => {
    ...
    const getAlertBadge = (level: string | null) => {
        ...
        return (
            <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border ${colors[level] || ''}`}>
                {level}
            </span>
        );
    };
            ...
            cell: ({ row }) => (
                <div>
                    {row.original.insurance.expiryDate ? (
                        <>
                            <div className="text-sm">{formatDate(row.original.insurance.expiryDate)}</div>
                            <div className="text-xs text-gray-500">
                                {row.original.insurance.daysRemaining !== null && row.original.insurance.daysRemaining >= 0
            ...
            cell: ({ row }) => (
                <div>
                    {row.original.permit.expiryDate ? (
                        <>
                            <div className="text-sm">{formatDate(row.original.permit.expiryDate)}</div>
                            <div className="text-xs text-gray-500">
                                {row.original.permit.daysRemaining !== null && row.original.permit.daysRemaining >= 0
    ...
    const alertDistributionData = reportData?.summary.alertDistribution
        ? [
            { name: 'Expired', value: reportData.summary.alertDistribution.expired, color: ALERT_COLORS.EXPIRED },
            { name: 'Critical', value: reportData.summary.alertDistribution.critical, color: ALERT_COLORS.CRITICAL },
            { name: 'High', value: reportData.summary.alertDistribution.high, color: ALERT_COLORS.HIGH },
            { name: 'Medium', value: reportData.summary.alertDistribution.medium, color: ALERT_COLORS.MEDIUM },
            { name: 'Low', value: reportData.summary.alertDistribution.low, color: ALERT_COLORS.LOW },
            ...
            <Card>
                <CardContent className="pt-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label className="text-sm font-medium mb-2 block">Days Threshold</label>
