...
import { DollarSign, Car, Activity, AlertTriangle, Wrench, BarChart as BarChartIcon } from 'lucide-react';
...
export const FleetExecutiveReport = ({ startDate: initialStartDate, endDate: initialEndDate }: FleetExecutiveReportProps) => {
    ...
    const COLORS = ['#FF5F00', '#10b981', '#3b82f6', '#f59e0b', '#8b5cf6', '#ec4899'];
        ...
        <div className="container mx-auto p-4 md:p-6 space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        {data.metadata.dateRange.startDate} to {data.metadata.dateRange.endDate}
                    </p>
                </div>
                <ExportButton
                    apiUrl={getExportUrl()}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data.operationalMetrics.damageReports}</div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{data.operationalMetrics.maintenanceRecords}</div>
                        <p className="text-xs text-muted-foreground mt-1">
                        ...
                        {loading ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={data.branchPerformance}>
                                    ...
                                    <Tooltip />
                                    <Legend />
                                    <Bar yAxisId="left" dataKey="totalRevenue" name="Revenue" fill="#FF5F00" />
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {data.categoryPerformance.map((category, index) => (
                                <div key={category.categoryName} className="flex items-center justify-between p-3 rounded-lg border">
                                    <div className="flex items-center gap-3">
                                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length] }} />
                                        <div>
                                            <p className="font-medium">{category.categoryName}</p>
                                            <p className="text-sm text-muted-foreground">{category.totalVehicles} vehicles</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-semibold">₹{category.totalRevenue.toLocaleString('en-IN')}</p>
                                        <p className="text-sm text-muted-foreground">{category.totalBookings} bookings</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                                ...
                                {data.branchPerformance.map((branch) => (
                                    <tr key={branch.branchId} className="border-b hover:bg-muted/50">
                                        ...
                                        <td className="p-2 text-right">
                                            ₹{Math.round(branch.averageBookingValue).toLocaleString('en-IN')}
                                        </td>
                                        <td className="p-2 text-right">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${branch.utilization >= 70 ? 'bg-green-100 text-green-700' :
                                                branch.utilization >= 40 ? 'bg-yellow-100 text-yellow-700' :
