...
import { Car, TrendingUp, DollarSign, AlertTriangle, Calendar, FileText } from 'lucide-react';
...
import type { VehicleHistoryData, VehicleHistoryBooking } from '@/services/admin.service';
...
export const VehicleHistoryReport = ({ vehicleId }: VehicleHistoryReportProps) => {
    ...
    const bookingColumns: ColumnDef<VehicleHistoryBooking>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="text-sm">
                    ...
                    <div className="text-xs text-gray-500">to {formatDate(row.original.endDate)}</div>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <span
                    className={`text-sm ${row.original.returnCondition === 'Good' ? 'text-green-600' : 'text-red-600'
                        }`}
                >
    ...
    const revenueTrendData = reportData?.bookingHistory.data
        .slice(0, 10)
        .reverse()
        .map((booking, index) => ({
            index: index + 1,
        ...
        <div className="container mx-auto p-4 md:p-6 space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                        {reportData ? `${reportData.vehicle.make} ${reportData.vehicle.model}` : ''}
                    </p>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <MetricCard
                        title="Total Revenue"
                        value={abbreviateAmount(reportData?.performanceMetrics.totalRevenue || 0)}
                        icon={DollarSign}
                        variant="revenue"
                        isLoading={isLoading}
                    />
                    <MetricCard
                        ...
                        value={reportData?.performanceMetrics.totalBookings.toString() || '0'}
                        icon={FileText}
                        variant="bookings"
                        isLoading={isLoading}
                    />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold">{formatCurrency(reportData?.performanceMetrics.totalRevenue || 0)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-sm text-gray-600">Avg Revenue/Booking:</span>
                                    <span className="font-semibold">{formatCurrency(reportData?.performanceMetrics.avgRevenuePerBooking || 0)}</span>
                                </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold text-orange-600">
                                        {formatCurrency(reportData?.performanceMetrics.totalMaintenanceCost || 0)}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-sm text-gray-600">Damage Cost:</span>
                                    <span className="font-semibold text-red-600">
                                        {formatCurrency(reportData?.performanceMetrics.totalDamageCost || 0)}
                                    </span>
                                </div>
                                <div className="flex justify-between border-t pt-3">
                                    <span className="text-sm font-medium">Net Profitability:</span>
                                    <span className={`font-bold text-lg ${(reportData?.performanceMetrics.netProfitability || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                        {formatCurrency(reportData?.performanceMetrics.netProfitability || 0)}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        {reportData.upcomingAlerts.insuranceExpiry && (
                            <div className={`flex items-center gap-3 p-3 rounded-md ${reportData.upcomingAlerts.insuranceExpiry.daysRemaining <= 30 ? 'bg-red-50 border border-red-200' : 'bg-yellow-50 border border-yellow-200'}`}>
                                <Calendar className={`h-5 w-5 flex-shrink-0 ${reportData.upcomingAlerts.insuranceExpiry.daysRemaining <= 30 ? 'text-red-600' : 'text-yellow-600'}`} />
                                <div>
                                    <p className="text-sm font-medium">Insurance Expiry</p>
                                    <p className="text-xs text-gray-600">
                                        {formatDate(reportData.upcomingAlerts.insuranceExpiry.date)} ({reportData.upcomingAlerts.insuranceExpiry.daysRemaining} days)
                                    </p>
                                </div>
                            </div>
                        )}
            ...
            <Tabs defaultValue="bookings" className="w-full">
                ...
                <TabsContent value="bookings" className="space-y-4">
                    ...
                    {revenueTrendData.length > 0 && (
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-base font-semibold">Revenue Trend (Last 10 Bookings)</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <ResponsiveContainer width="100%" height={250}>
                                    <LineChart data={revenueTrendData}>
                                        ...
                                        <Line type="monotone" dataKey="revenue" stroke="#FF5F00" strokeWidth={2} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                            ...
                            {isLoading ? (
                                ...
                                <div className="space-y-3">
                                    {reportData.damageHistory.map((damage) => (
                                        <div key={damage.id} className="border rounded-md p-4 bg-red-50">
                                            <div className="flex justify-between items-start mb-2">
                                                <div>
                                                    <p className="font-medium text-red-900">
                                                        {damage.severity ? `${damage.severity} damage` : 'Damage reported'}
                                                    </p>
                                                    <p className="text-xs text-gray-600">
                                                        Booking: {damage.bookingId} • {damage.customerName}
                                                    </p>
                                                    <p className="text-xs text-gray-500">{formatDate(damage.reportedDate)}</p>
                                                </div>
