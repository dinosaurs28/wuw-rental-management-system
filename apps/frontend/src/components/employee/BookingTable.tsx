    ...
    <div className="grid gap-4 md:hidden">
      {bookings.map((booking) => (
        <Card
          ...
          className="overflow-hidden shadow-sm border-muted"
        >
            ...
            <div className="flex gap-3 p-3 bg-muted/20 rounded-lg border border-muted/50">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm truncate">
                  {booking.items[0]?.vehicle.make}{" "}
                  {booking.items[0]?.vehicle.model}
                </p>
          ...
          {bookings.map((booking) => (
            <TableRow key={booking.publicId} className="hover:bg-muted/10 h-16">
              ...
              <TableCell className="text-sm">
                <div className="flex flex-col">
                  <span className="font-medium">
                    {format(new Date(booking.startAt), "MMM dd")}
                  </span>
              ...
              <TableCell className="text-sm">
                <div className="flex flex-col">
                  <span className="font-medium">
                    {format(new Date(booking.endAt), "MMM dd")}
                  </span>
                  <span className="text-xs text-muted-foreground">
...
function AvatarFallback({ name }: { name: string }) {
  ...
  return (
    <div
      className={cn(
        "h-9 w-9 rounded-full flex items-center justify-center font-bold text-xs shrink-0 select-none",
        colorClass,
      )}
    >
      {initials}
