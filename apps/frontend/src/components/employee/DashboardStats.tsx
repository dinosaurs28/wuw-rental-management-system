...
function StatsCard({ title, value, icon: Icon, trend, alert, active }: StatsCardProps) {
    return (
        <div className={cn(
            ...
            <div className="flex justify-between items-start mb-4">
                <div className={cn(
                    ...
                    alert ? "bg-orange-100 text-orange-600" : "bg-muted text-muted-foreground",
            ...
            <div className="space-y-1">
                <p className={cn("text-sm font-medium", alert ? "text-orange-700" : "text-muted-foreground")}>{title}</p>
                <div className="flex items-baseline gap-2">
                    <h3 className="text-3xl font-bold tracking-tight">{value}</h3>
                    {alert && <span className="text-sm font-medium text-orange-600">Pending Approvals</span>}
                </div>
            </div>
        </div>
    );
