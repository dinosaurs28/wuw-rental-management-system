...
export const DamageReports = ({ reports = [], isLoading = false, onLoadMore, hasMore = false }: DamageReportsProps) => {
            ...
            <CardContent className="p-0">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 divide-x divide-y md:divide-y-0 text-center border-b">
                    <div className="p-4 bg-neutral-50/50 flex flex-col items-center justify-center gap-1">
                        ...
                        <span className="text-xs text-neutral-500 font-medium uppercase tracking-wide">Shown</span>
                    </div>
                    <div className="p-4 bg-neutral-50/50 flex flex-col items-center justify-center gap-1">
                    ...
                    <div className="p-4 bg-neutral-50/50 flex flex-col items-center justify-center gap-1">
                        <span className="text-2xl font-bold text-blue-600">{reports.filter(r => r.status === 'PENDING').length}</span>
                        <span className="text-xs text-neutral-500 font-medium uppercase tracking-wide">Pending</span>
                    </div>
                </div>
                        ...
                        reports.map((report) => (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="text-sm font-semibold truncate text-neutral-900">#{report.id} - {report.vehicleName}</h4>
                                ...
                                <div>
                                    <span className={`text-[10px] font-bold px-2 py-1 rounded-full border ${report.status === 'PENDING' ? 'bg-blue-50 text-blue-700 border-blue-200' :
