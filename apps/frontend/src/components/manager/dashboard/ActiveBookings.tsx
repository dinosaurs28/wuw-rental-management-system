...
export const ActiveBookings = ({ bookings = [], isLoading = false }: ActiveBookingsProps) => {
    ...
    return (
        <Card className="border shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between pb-4">
                ...
                <Button variant="ghost" size="sm" className="gap-1 text-orange-600 hover:text-orange-700 hover:bg-orange-50" asChild>
                            ...
                            {isLoading ? (
                                ...
                                bookings.map((booking) => (
                                    <TableRow key={booking.id} className="group">
                                        ...
                                        <TableCell className="text-neutral-600">{booking.vehicleName}</TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 uppercase text-[10px] tracking-wide">
                                                Active
                                            </Badge>
                                        </TableCell>
                                        <TableCell className="text-right">
                                            <Button variant="outline" size="sm" className="h-8 text-xs bg-white" asChild>
                                                <Link to={`/manager/bookings/${booking.id}`}>Details</Link>
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                                ))
