...
type BookingStatus = 'HOLD' | 'CONFIRMED' | 'PICKED_UP' | 'RETURNED' | 'CANCELLED' | 'HOLD_EXPIRED';
...
export function InvoiceDownloadButton({ bookingId, bookingStatus }: InvoiceDownloadButtonProps) {
    ...
    const shouldShowButton = bookingStatus === 'CONFIRMED' || bookingStatus === 'RETURNED';
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const response = await apiClient.post('/invoices/download', { bookingId });
    ...
    const pollForInvoice = async (invoiceId: number) => {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                setMessage(data.message || `Generating... ${Math.round(data.progress || 0)}%`);
        ...
        setProgress(0);
        setMessage('');
        throw new Error('Invoice generation is taking longer than expected. Please refresh and try again.');
    };
        ...
        >
            {loading ? (
                <>
                    ...
                    {progress > 0 && <span className="text-xs ml-1">({Math.round(progress)}%)</span>}
                </>
            ) : (
                <>
