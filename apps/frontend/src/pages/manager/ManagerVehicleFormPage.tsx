...
const vehicleSchema = z.object({
    ...
    year: z.coerce.number().min(1900).max(new Date().getFullYear() + 1), // Kept for UI, might not be saved by backend
    ...
    images: z.array(z.custom<string | File>()).min(1, "At least one image is required"),
});
            ...
            const loadVehicle = async () => {
                try {
                    ...
                    form.reset({
                        ...
                        policyNumber: vehicle.policyNumber || latestInsurance?.policyNumber || "",
                        provider: vehicle.provider || latestInsurance?.provider || "",
                        images: vehicle.images?.filter(img => !img.isThumbnail).map(img => img.file.url) || [],
                    });
                } catch (error) {
    ...
    const onSubmit = async (data: VehicleFormValues) => {
        ...
        try {
            ...
            if (isEditMode) {
                ...
                const currentImageUrls = data.images.filter(img => typeof img === 'string') as string[];
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            <div className="bg-white p-6 rounded-lg border shadow-sm space-y-6">
                                <h2 className="text-xl font-semibold text-neutral-900">Vehicle Details</h2>
                                <Separator />
                            ...
                            <div className="bg-white p-6 rounded-lg border shadow-sm space-y-6">
                                <h2 className="text-xl font-semibold text-neutral-900">Pricing & Insurance</h2>
                                <Separator />
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <FormField
                                        control={form.control}
                            ...
                            {/* Section 3: Images */}
                            <div className="bg-white p-6 rounded-lg border shadow-sm space-y-6">
                                <h2 className="text-xl font-semibold text-neutral-900">Vehicle Images</h2>
                                <Separator />
                                <FormField
                                    control={form.control}
                                    name="images"
