...
export const EmployeeVehicleDetailsPage = () => {
  ...
  const [selectedKycId, setSelectedKycId] = useState<string | null>(
    customerKycId,
  );
  ...
  const fetchKycDocuments = async () => {
    ...
    setKycError(null);
    try {
      const response = await kycService.getCustomerKyc(
    ...
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <p className="text-zinc-600 mb-6">
          The vehicle you are looking for does not exist or is unavailable.
        </p>
  ...
  const vehicleFeatures = [
    ...(vehicle.seats
      ...
      : []),
    ...(vehicle.fuelType ? [{ icon: Fuel, label: vehicle.fuelType }] : []),
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <VehicleImageGallery
              images={vehicle.images.map((img) =>
                typeof img === "string" ? img : img.file.url,
              )}
            ...
            <div className="hidden lg:block space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                <Badge
                  variant="secondary"
                  className="px-3 py-1 text-sm bg-zinc-100 text-zinc-700 hover:bg-zinc-200"
                >
                  {vehicle.category?.name}
                </Badge>
                {vehicleFeatures.map((feature, index) => (
            ...
            <div
              id="kyc-section"
              className="space-y-4 pt-6 border-t border-zinc-200"
            >
