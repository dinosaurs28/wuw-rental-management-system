...
import {
  ...
  PaymentMethodChart,
} from "@/components/admin/AdminRevenueCharts";
...
export const AdminDashboardPage = () => {
  ...
  const fetchAnalyticsData = async (
    ...
    try {
      ...
      const [revenueReport, trendData, catData, paymentData] =
        await Promise.all([
  ...
  const totalFleet = branches.reduce(
    (acc, branch) => acc + (branch._count?.vehicles || 0),
    0,
  );
  const activeUsers = branches.reduce(
    (acc, branch) => acc + (branch._count?.users || 0),
    0,
  );
      ...
      <div className="mb-6">
          ...
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink
                ...
                className="text-muted-foreground hover:text-[#FF5F00]"
              >
                Admin
              </BreadcrumbLink>
            </BreadcrumbItem>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          ...
          <Button
            ...
            className="h-11 bg-neutral-900 hover:bg-neutral-800 text-white shadow-sm px-6 w-full sm:w-auto"
          >
        ...
        <Card className="border border-neutral-200 shadow-sm rounded-xl overflow-hidden bg-[#FF5F00] text-white">
          ...
          <CardContent>
            ...
            <div className="space-y-2">
              <Button
                variant="secondary"
                className="w-full justify-start bg-white text-[#FF5F00] hover:bg-neutral-50 border-0 h-10 font-medium"
              >
        ...
        <Card className="border border-neutral-200 shadow-sm rounded-xl bg-white">
          ...
          <CardContent className="space-y-3 pt-0">
            ...
            <div className="p-3 bg-neutral-50 rounded-lg border border-neutral-100">
              <p className="text-sm text-neutral-700">
                <span className="font-semibold">{branches.length}</span> active
                branch locations
              </p>
            </div>
          </CardContent>
