...
export const GlobalReports = () => {
    ...
    const fetchStats = async () => {
        try {
            ...
            if (selectedCategories.length > 0) params.append('categoryId', selectedCategories[0]); // Taking first for now as backend simplified
            ...
            const response = await apiClient.get(`/admin/dashboard/reports/global-kpi?${params.toString()}`);
    ...
    const handleResetFilters = () => {
        ...
        setTimeout(() => { // slight delay to allow state verification or just call with defaults 
    ...
    const fetchStatsWithDefaults = async () => {
        ...
        try {
            ...
            const response = await apiClient.get(`/api/admin/dashboard/reports/global-kpi?${params.toString()}`);
    ...
    const isFiltered = selectedBranchId !== 'all' || selectedCategories.length > 0 || dateRange.start !== null;
    ...
    return (
        <div className="max-w-[1440px] mx-auto px-4 md:px-6 pt-8 pb-12 space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-neutral-900">Global Reports</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="animate-in slide-in-from-bottom-3 duration-500 delay-100 fill-mode-backwards">
                    <KPICard
                        ...
                        value={loading ? "0" : formatCurrency(data?.revenueThisMonth.current || 0)}
                        icon={DollarSign}
                        colorTheme="emerald"
                        loading={loading}
                ...
                <div className="animate-in slide-in-from-bottom-3 duration-500 delay-150 fill-mode-backwards">
                    <KPICard
                        ...
                        value={loading ? "0" : formatNumber(data?.activeBookings.count || 0)}
                        icon={Activity}
                        colorTheme="violet"
                        loading={loading}
                ...
                <div className="animate-in slide-in-from-bottom-3 duration-500 delay-200 fill-mode-backwards">
                    <KPICard
                        ...
                        value={loading ? "0" : formatCurrency(data?.pendingRevenue.amount || 0)}
                        icon={CreditCard}
                        colorTheme="amber"
                        loading={loading}
                        subtitle={
                ...
                <div className="animate-in slide-in-from-bottom-3 duration-500 delay-300 fill-mode-backwards">
                    <KPICard
                        ...
                        value={loading ? "0" : formatNumber(data?.totalCustomers.count || 0)}
                        icon={Users}
                        colorTheme="cyan"
                        loading={loading}
                ...
                <div className="animate-in slide-in-from-bottom-3 duration-500 delay-500 fill-mode-backwards">
                    <KPICard
                        ...
                        value={loading ? "0" : formatCurrency(data?.avgBookingValue.amount || 0)}
                        icon={TrendingUp}
                        colorTheme="rose"
                        loading={loading}
