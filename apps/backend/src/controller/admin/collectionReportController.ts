...
export const GetCollectionReport = async (req: Request, res: Response) => {
    try {
        const { startDate, endDate, branchId, paymentMethod = 'all', export: exportFormat } = req.query;
        ...
        if (paymentMethod && paymentMethod !== 'all') {
            paymentFilter.method = paymentMethod === 'ONLINE' ? 'ONLINE_RAZORPAY' : paymentMethod;
        }
        ...
        payments.forEach((payment) => {
            ...
            if (branchBreakdown[branchId]) {
                ...
                if (payment.method in branchBreakdown[branchId].byMethod) {
                    branchBreakdown[branchId].byMethod[payment.method] += Number(payment.amount);
                }
            }
        });
