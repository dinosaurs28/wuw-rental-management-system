...
export const CreateBranch = async (req: Request, res: Response) => {
  try {
    ...
    // Auto-generate a temporary password for the manager
...
export const EditBranch = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (
        (data.managerName || data.managerEmail || data.managerPassword) &&
        manager
      ) {
...
export const DeleteBranch = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(StatusCode.OK).json({
      message:
        "Branch deleted successfully. Associated data will be removed in the background.",
    });
  } catch (error) {
