...
export const getGlobalKpiStats = async (req: Request, res: Response) => {
  try {
    ...
    const fleetUtilizationCurrent =
      totalActiveVehicles > 0
        ? (rentedVehicles / totalActiveVehicles) * 100
        : 0;
    const fleetUtilizationLast =
      totalActiveVehicles > 0
        ? (rentedVehiclesLastPeriod / totalActiveVehicles) * 100
        : 0;
    const fleetUtilizationTrend =
      fleetUtilizationCurrent - fleetUtilizationLast;
    ...
    const revenueThisPeriod = Number(
      revenueThisPeriodAgg._sum?.totalFinal || 0,
    );
    ...
    const revenueLastPeriod = Number(
      revenueLastPeriodAgg._sum?.totalFinal || 0,
    );
    ...
    const activeBookingsTrend =
      activeBookingsLastCount > 0
        ? ((activeBookingsCount - activeBookingsLastCount) /
            activeBookingsLastCount) *
          100
    ...
    const pendingRevenueAmount = Number(
      pendingRevenueAgg._sum?.totalFinal || 0,
    );
    ...
    const avgBookingValue =
      totalBookingsPeriod > 0 ? totalRevenuePeriod / totalBookingsPeriod : 0;
    ...
    const avgBookingValuePrev =
      totalBookingsPrev > 0 ? totalRevenuePrev / totalBookingsPrev : 0;

    const avgBookingValueTrend =
      avgBookingValuePrev > 0
        ? ((avgBookingValue - avgBookingValuePrev) / avgBookingValuePrev) * 100
        : 0;

    // Debug Logging
    console.log(
      `[KPI] Range: ${currentPeriodStart.toISOString()} - ${currentPeriodEnd.toISOString()}`,
    );
