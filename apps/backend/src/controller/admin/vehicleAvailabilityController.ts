...
export const GetVehicleAvailability = async (req: Request, res: Response) => {
    try {
        const { startDate, endDate, branchId, categoryId, export: exportFormat } = req.query;
        ...
        const vehicleData = vehicles.map((vehicle) => {
            ...
            const utilizationRate = totalDays > 0 ? ((bookedDaysInRange / totalDays) * 100).toFixed(1) : '0.0';
            ...
            const isCurrentlyRented = bookings.some(
                (b) => new Date(b.startDate) <= now && new Date(b.endDate) >= now && b.status === 'PICKED_UP'
            );
        ...
        const categoryBreakdown = categories.map((category) => {
            const categoryVehicles = vehicleData.filter((v) => v.category === category.name);
            const total = categoryVehicles.length;
            const available = categoryVehicles.filter((v) => v.currentStatus === 'AVAILABLE').length;
            const rented = categoryVehicles.filter((v) => v.currentStatus === 'RENTED').length;
            const maintenance = categoryVehicles.filter((v) => v.currentStatus === 'MAINTENANCE').length;
            const inactive = categoryVehicles.filter((v) => v.currentStatus === 'INACTIVE').length;

            const avgUtilization =
                total > 0
                    ? (categoryVehicles.reduce((sum, v) => sum + v.utilizationRate, 0) / total).toFixed(1)
                    : '0.0';
