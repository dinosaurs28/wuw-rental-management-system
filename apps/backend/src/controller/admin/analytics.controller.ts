...
export const GetRevenueTrends = async (req: Request, res: Response) => {
  try {
    ...
    const groupedData = new Map<
      string,
      { revenue: number; count: number; branchId?: number }
    >();
      ...
      .map((cat) => {
        ...
        return {
          ...
          avgRevenuePerVehicle:
            vehicleCount > 0 ? cat.totalRevenue / vehicleCount : 0,
        };
      })
...
export const GetPaymentMethodBreakdown = async (
  ...
  try {
    ...
    const totalRevenue = Array.from(methodMap.values()).reduce(
    ...
    const data = Array.from(methodMap.entries())
      .map(([method, stats]) => ({
        ...
        percentageShare:
          totalRevenue > 0 ? (stats.totalRevenue / totalRevenue) * 100 : 0,
      }))
