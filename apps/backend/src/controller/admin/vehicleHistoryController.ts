...
export const GetVehicleHistory = async (req: Request, res: Response) => {
    try {
        ...
        // Net profitability
        const netProfitability = totalRevenue - totalMaintenanceCost - totalDamageCost;
        ...
        const utilizationRate = vehicleAge > 0 ? ((totalDays / vehicleAge) * 100).toFixed(1) : '0.0';
        ...
        const avgRevenuePerBooking = totalBookingsCount > 0
                ...
                insuranceExpiry: vehicle.insuranceExpiry
                    ? {
                        ...
                        daysRemaining: Math.floor(
                            (vehicle.insuranceExpiry.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
                        ),
                    }
                    : null,
