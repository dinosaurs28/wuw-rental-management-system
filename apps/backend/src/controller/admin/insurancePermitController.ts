    ...
    try {
        ...
        const processedVehicles = vehicles.map((vehicle) => {
            const insuranceDaysRemaining = Math.ceil(
                (vehicle.insuranceExpiry.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24)
            );
        ...
        const criticalAlerts = processedVehicles.filter(
            (v) => v.overallAlertLevel === 'CRITICAL' || v.overallAlertLevel === 'EXPIRED'
        ).length;
        ...
        const alertDistribution = {
            expired: processedVehicles.filter((v) => v.overallAlertLevel === 'EXPIRED').length,
            critical: processedVehicles.filter((v) => v.overallAlertLevel === 'CRITICAL').length,
            high: processedVehicles.filter((v) => v.overallAlertLevel === 'HIGH').length,
            medium: processedVehicles.filter((v) => v.overallAlertLevel === 'MEDIUM').length,
            low: processedVehicles.filter((v) => v.overallAlertLevel === 'LOW').length,
        };
        ...
        processedVehicles.forEach((vehicle) => {
            ...
            if (vehicle.overallAlertLevel === 'CRITICAL' || vehicle.overallAlertLevel === 'EXPIRED') {
