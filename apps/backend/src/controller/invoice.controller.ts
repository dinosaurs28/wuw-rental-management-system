...
import { queueInvoiceGeneration, getInvoiceJobStatus } from '../utils/invoice-generation.queue.js';
...
export async function downloadInvoice(req: Request, res: Response) {
    try {
        ...
        if (booking.invoice.invoicePdfFile?.url) {
            console.log(`[Invoice Controller] Cache HIT - Invoice ${booking.invoice.id}`);
        ...
        if (jobStatus && ['waiting', 'active'].includes(jobStatus.state)) {
            console.log(`[Invoice Controller] Generation in progress - Invoice ${booking.invoice.id}`);
        ...
        console.log(`[Invoice Controller] Queuing generation - Invoice ${booking.invoice.id}`);
