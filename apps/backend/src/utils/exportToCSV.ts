...
export const exportSalesReportToCSV = (
    ...
    const fields = [
        'Booking ID', 'Booking Date', 'Customer Name', 'Customer Phone', 'Customer Email',
        'Vehicle', 'Reg No', 'Start Date', 'End Date', 'Days',
        'Base Amount', 'Discount', 'Tax', 'Total Amount', 'Amount Paid', 'Balance',
        'Payment Method', 'Payment Status', 'Status', 'Branch', 'Created By',
    ];
...
export const exportVehicleHistoryToCSV = (
    ...
    const fields = [
        'Booking ID', 'Customer Name', 'Customer Phone', 'Start Date', 'End Date',
        'Days', 'Revenue', 'Deposit', 'Status', 'Damage Reported', 'Return Condition',
    ];
...
export const exportCollectionToCSV = (
    ...
    const fields = ['Payment ID', 'Booking ID', 'Date', 'Customer', 'Phone', 'Amount', 'Method', 'Branch', 'Type'];
    const csv = arrayToCSV(csvData, fields);
    sendCSVFile(res, csv, filename);
};
...
export const exportFleetExecutiveToCSV = (
    ...
    const fields = ['Branch', 'Total Vehicles', 'Active Vehicles', 'Total Bookings', 'Total Revenue', 'Avg Booking Value', 'Utilization'];
    const csv = arrayToCSV(csvData, fields);
    sendCSVFile(res, csv, filename);
};
...
export const exportGSTToCSV = (
    ...
    const fields = ['Invoice Number', 'Date', 'Customer', 'Taxable Amount', 'CGST', 'SGST', 'IGST', 'Total GST', 'Total Amount', 'GST Rate', 'Type'];
    const csv = arrayToCSV(csvData, fields);
    sendCSVFile(res, csv, filename);
};
...
export const exportVehicleAvailabilityToCSV = (
    ...
    const csvData = data.vehicles.map((vehicle: any) => ({
        ...
        'Upcoming Bookings': vehicle.bookings.filter((b: any) => new Date(b.startDate) > new Date()).length
    }));

    const fields = ['Reg No', 'Make', 'Model', 'Category', 'Branch', 'Status', 'Current Status', 'Utilization', 'Booked Days', 'Total Days', 'Upcoming Bookings'];
    const csv = arrayToCSV(csvData, fields);
    sendCSVFile(res, csv, filename);
};
