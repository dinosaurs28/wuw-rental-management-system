...
export const exportDailySummaryToExcel = async (
    ...
    addTitleRow(summarySheet, `Daily Summary - ${formatDateForExcel(data.metadata.date)}`, 4);

    summarySheet.addRow(['Branch', data.metadata.branch]);
    summarySheet.addRow(['Generated At', formatDateForExcel(data.metadata.generatedAt)]);
    summarySheet.addRow([]);

    // Revenue Section
    summarySheet.addRow(['REVENUE METRICS']);
    createHeaderRow(summarySheet, ['Metric', 'Amount'], (summarySheet.lastRow?.number ?? 0) + 1);
    summarySheet.addRow(['New Bookings', formatCurrencyForExcel(data.revenue.newBookings)]);
    summarySheet.addRow(['Advance Collected', formatCurrencyForExcel(data.revenue.advanceCollected)]);
    summarySheet.addRow(['Deposit Collected', formatCurrencyForExcel(data.revenue.depositCollected)]);
    summarySheet.addRow(['Total Collected', formatCurrencyForExcel(data.revenue.totalCollected)]);
    summarySheet.addRow([]);
    ...
    createHeaderRow(summarySheet, ['Payment Method', 'Amount', 'Count'], (summarySheet.lastRow?.number ?? 0) + 1);
...
export const exportSalesReportToExcel = async (
    ...
    summarySheet.addRow(['Period', `${formatDateForExcel(data.metadata.period.start)} to ${formatDateForExcel(data.metadata.period.end)}`]);
    summarySheet.addRow(['Generated At', formatDateForExcel(data.metadata.generatedAt)]);
    summarySheet.addRow([]);

    createHeaderRow(summarySheet, ['Metric', 'Value'], (summarySheet.lastRow?.number ?? 0) + 1);
    summarySheet.addRow(['Total Revenue', formatCurrencyForExcel(data.summary.totalRevenue)]);
    summarySheet.addRow(['Total Bookings', data.summary.totalBookings]);
    summarySheet.addRow(['Average Booking Value', formatCurrencyForExcel(data.summary.averageBookingValue)]);
    summarySheet.addRow(['Tax Collected', formatCurrencyForExcel(data.summary.taxCollected)]);
    summarySheet.addRow(['Deposits Collected', formatCurrencyForExcel(data.summary.depositsCollected)]);
    summarySheet.addRow(['Net Revenue', formatCurrencyForExcel(data.summary.netRevenue)]);
    ...
    const headers = [
        'Booking ID', 'Date', 'Customer', 'Phone', 'Vehicle',
        'Start Date', 'End Date', 'Days', 'Amount', 'Status', 'Branch'
    ];
...
export const exportVehicleHistoryToExcel = async (
    ...
    summarySheet.addRow(['Vehicle', `${data.vehicle.make} ${data.vehicle.model}`]);
    ...
    createHeaderRow(summarySheet, ['Performance Metric', 'Value'], (summarySheet.lastRow?.number ?? 0) + 1);
    summarySheet.addRow(['Total Revenue', formatCurrencyForExcel(data.performanceMetrics.totalRevenue)]);
    summarySheet.addRow(['Total Bookings', data.performanceMetrics.totalBookings]);
    summarySheet.addRow(['Utilization Rate', `${data.performanceMetrics.utilizationRate}%`]);
    summarySheet.addRow(['Total Maintenance Cost', formatCurrencyForExcel(data.performanceMetrics.totalMaintenanceCost)]);
    summarySheet.addRow(['Net Profitability', formatCurrencyForExcel(data.performanceMetrics.netProfitability)]);
    ...
    const bookingHeaders = [
        'Booking ID', 'Customer', 'Phone', 'Start Date', 'End Date',
        'Days', 'Revenue', 'Status', 'Return Condition'
    ];
    ...
    const maintenanceHeaders = ['Date', 'Description', 'Cost', 'Odometer', 'Serviced By'];
...
export const exportCollectionToExcel = async (
    ...
    // Sheet 1: Summary
    const summarySheet = workbook.addWorksheet('Summary');
    addTitleRow(summarySheet, `Collection Report - ${formatDateForExcel(data.metadata.dateRange.startDate)} to ${formatDateForExcel(data.metadata.dateRange.endDate)}`, 4);
    ...
    createHeaderRow(summarySheet, ['Metric', 'Value'], (summarySheet.lastRow?.number ?? 0) + 1);
    summarySheet.addRow(['Total Collected', formatCurrencyForExcel(data.summary.totalCollected)]);
    summarySheet.addRow(['Total Transactions', data.summary.totalTransactions]);
    summarySheet.addRow(['Average Transaction', formatCurrencyForExcel(data.summary.averageTransaction)]);
    summarySheet.addRow([]);

    summarySheet.addRow(['METHOD BREAKDOWN']);
    createHeaderRow(summarySheet, ['Method', 'Amount', 'Count'], (summarySheet.lastRow?.number ?? 0) + 1);
    Object.entries(data.summary.methodBreakdown).forEach(([method, values]: [string, any]) => {
        summarySheet.addRow([method, formatCurrencyForExcel(values.amount), values.count]);
    });
    ...
    const headers = ['Payment ID', 'Booking ID', 'Date', 'Customer', 'Phone', 'Amount', 'Method', 'Branch'];
...
export const exportFleetExecutiveToExcel = async (
    ...
    addTitleRow(summarySheet, `Fleet Executive Report - ${formatDateForExcel(data.metadata.dateRange.startDate)} to ${formatDateForExcel(data.metadata.dateRange.endDate)}`, 4);

    summarySheet.addRow(['Generated At', formatDateForExcel(data.metadata.generatedAt)]);
    summarySheet.addRow([]);

    summarySheet.addRow(['KEY PERFORMANCE INDICATORS']);
    createHeaderRow(summarySheet, ['KPI', 'Value'], (summarySheet.lastRow?.number ?? 0) + 1);
    ...
    summarySheet.addRow(['Total Revenue', formatCurrencyForExcel(data.kpis.totalRevenue)]);
    summarySheet.addRow(['Avg Booking Value', formatCurrencyForExcel(data.kpis.averageBookingValue)]);
    summarySheet.addRow(['Fleet Utilization', `${data.kpis.fleetUtilization.toFixed(1)}%`]);
    summarySheet.addRow(['Revenue Per Vehicle', formatCurrencyForExcel(data.kpis.revenuePerVehicle)]);
    summarySheet.addRow([]);

    summarySheet.addRow(['OPERATIONAL METRICS']);
    createHeaderRow(summarySheet, ['Metric', 'Value'], (summarySheet.lastRow?.number ?? 0) + 1);
    summarySheet.addRow(['Damage Reports', data.operationalMetrics.damageReports]);
    summarySheet.addRow(['Maintenance Records', data.operationalMetrics.maintenanceRecords]);
    summarySheet.addRow(['Damage Rate', `${data.operationalMetrics.damageReportRate.toFixed(1)}%`]);
    summarySheet.addRow(['Maintenance Rate', `${data.operationalMetrics.maintenanceRate.toFixed(1)}%`]);

    autoFitColumns(summarySheet);

    // Sheet 2: Branch Performance
    const branchSheet = workbook.addWorksheet('Branch Performance');
    const branchHeaders = ['Branch', 'Total Vehicles', 'Active', 'Bookings', 'Revenue', 'Avg Booking Value', 'Utilization'];
...
export const exportGSTToExcel = async (
    ...
    addTitleRow(summarySheet, `GST Report - ${formatDateForExcel(data.metadata.dateRange.startDate)} to ${formatDateForExcel(data.metadata.dateRange.endDate)}`, 4);
    ...
    summarySheet.addRow(['Total Taxable Amount', formatCurrencyForExcel(data.summary.totalTaxableAmount)]);
    summarySheet.addRow(['Total CGST', formatCurrencyForExcel(data.summary.totalCGST)]);
    summarySheet.addRow(['Total SGST', formatCurrencyForExcel(data.summary.totalSGST)]);
    summarySheet.addRow(['Total IGST', formatCurrencyForExcel(data.summary.totalIGST)]);
    summarySheet.addRow(['Total GST', formatCurrencyForExcel(data.summary.totalGST)]);
    summarySheet.addRow(['Total Invoice Amount', formatCurrencyForExcel(data.summary.totalInvoiceAmount)]);

    autoFitColumns(summarySheet);

    // Sheet 2: Invoices (B2B/B2C)
    const invoiceSheet = workbook.addWorksheet('Invoices');
    const invoiceHeaders = ['Invoice No', 'Date', 'Customer', 'GSTIN', 'Taxable Value', 'CGST', 'SGST', 'IGST', 'Total Tax', 'Total Amount', 'Place of Supply'];
...
export const exportVehicleAvailabilityToExcel = async (
    ...
    // Sheet 1: Summary
    const summarySheet = workbook.addWorksheet('Summary');
    addTitleRow(summarySheet, `Vehicle Availability - ${formatDateForExcel(data.metadata.dateRange.startDate)} to ${formatDateForExcel(data.metadata.dateRange.endDate)}`, 4);
    ...
    summarySheet.addRow(['Overall Utilization', `${data.summary.overallUtilization.toFixed(1)}%`]);

    autoFitColumns(summarySheet);

    summarySheet.addRow([]);
    summarySheet.addRow(['CATEGORY BREAKDOWN']);
    createHeaderRow(summarySheet, ['Category', 'Total', 'Available', 'Rented', 'Utilization'], (summarySheet.lastRow?.number ?? 0) + 1);
    ...
    const vehicleHeaders = ['Reg No', 'Make', 'Model', 'Category', 'Branch', 'Status', 'Current Status', 'Utilization', 'Booked Days'];
