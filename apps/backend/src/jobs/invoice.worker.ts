...
export function initInvoiceWorker(): void {
  ...
  invoiceWorker = new Worker(
    ...
    async (job: Job<InvoiceJobData>) => {
      ...
      try {
        console.log(
          `[Invoice Worker] Processing invoice ${invoiceId} for booking ${bookingId}`,
        );
        ...
        if (!invoiceNumber) {
          ...
          console.log(
            `[Invoice Worker] Generated invoice number: ${invoiceNumber}`,
          );
        }
        ...
        console.log(
          `[Invoice Worker] Transforming booking data for invoice ${invoiceNumber}`,
        );
        ...
        // Step 3: Generate PDF
        console.log(
          `[Invoice Worker] Generating PDF for invoice ${invoiceNumber}`,
        );
        ...
        // Step 4: Upload to R2
        console.log(
          `[Invoice Worker] Uploading PDF to R2 for invoice ${invoiceNumber}`,
        );
        ...
        console.log(
          `[Invoice Worker] ✅ Invoice ${invoiceNumber} (ID: ${invoiceId}) completed successfully`,
        );
        ...
        console.error(
          `[Invoice Worker] ❌ CRITICAL FAILURE for Invoice ${invoiceId}`,
        );
        console.error(
          "[Invoice Worker] Error Message:",
