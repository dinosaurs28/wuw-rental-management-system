...
function getRedisConfig() {
  ...
  if (!REDIS_URL) {
    throw new Error(
      "[BookingExpiry] REDIS_URL environment variable is required",
    );
  }
...
async function enableKeyspaceNotifications(): Promise<void> {
  try {
    ...
    if (redis.status !== "ready") {
      console.log(
        "[BookingExpiry] Waiting for Redis connection to be ready...",
      );
    ...
    console.error(
      "[BookingExpiry] Failed to enable keyspace notifications:",
      error,
    );
    throw error;
  }
...
async function handleBookingExpiry(bookingPublicId: string): Promise<void> {
  ...
  try {
    ...
    if (!booking) {
      console.log(
        `[BookingExpiry] Booking ${bookingPublicId} not found in database, skipping`,
      );
      return;
    }
    ...
    console.error(
      `[BookingExpiry] Error processing booking ${bookingPublicId}:`,
      error,
    );
  }
}
...
async function startExpiryListener(): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[BookingExpiry] Failed to subscribe to expired events:",
      err,
    );
  }
  ...
  redisSubscriber.on("message", async (channel, expiredKey) => {
    console.log(
      `[BookingExpiry] Received expired event for key: ${expiredKey}`,
    );
...
async function runFallbackCleanup(): Promise<void> {
  ...
  try {
    ...
    if (expiredBookings.length === 0) {
      console.log(
        "[BookingExpiry] No expired bookings found in fallback cleanup",
      );
      return;
    }

    console.log(
      `[BookingExpiry] Found ${expiredBookings.length} expired bookings in fallback cleanup`,
    );
...
export { handleBookingExpiry, runFallbackCleanup, initBookingExpiryWorker };
