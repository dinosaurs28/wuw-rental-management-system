...
export class DurationCalculatorService {
  ...
  static calculate(startAt: DateTime, endAt: DateTime): RentalDuration {
    ...
    if (actualHours <= 1) {
    ...
    } else if (actualHours <= 720) { // 30 days = 720 hours
      // Multi-day rental
      ...
      periodType = totalDays >= 30 ? RentalPeriodType.MONTHLY : RentalPeriodType.MULTI_DAY;
  ...
  static calculateExtraHours(
    ...
    const interval = Interval.fromDateTimes(expectedReturnTime, actualReturnTime);
