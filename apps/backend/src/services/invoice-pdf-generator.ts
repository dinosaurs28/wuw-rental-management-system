...
export async function generateInvoicePDF(
  ...
  return new Promise((resolve, reject) => {
    try {
      console.log(
        `[Invoice PDF Generator] Generating PDF for ${invoiceData.invoiceNumber}`,
      );
      ...
      doc.on("end", () => {
        ...
        console.log(
          `[Invoice PDF Generator] PDF generated successfully (${pdfBuffer.length} bytes)`,
        );
        resolve(pdfBuffer);
      });
      doc.on("error", reject);
      ...
      doc
        .fontSize(10)
        .font("Helvetica-Bold")
        .text(invoiceData.companyName, 50, 90);
      doc
        .fontSize(9)
        .font("Helvetica")
        .text(invoiceData.companyAddress, 50, 105, { width: 250 });
      ...
      doc
        .fontSize(10)
        .font("Helvetica-Bold")
        .text(`Invoice No: ${invoiceData.invoiceNumber}`, 350, 90);
      doc
        .fontSize(9)
        .font("Helvetica")
        .text(
          `Date: ${invoiceData.invoiceDate.toLocaleDateString("en-IN")}`,
          350,
          105,
        );
      ...
      doc
        .fontSize(9)
        .font("Helvetica")
        .text(invoiceData.customerName, 50, yPos + 15);
      ...
      doc
        .font("Helvetica")
        .text(
          `From: ${invoiceData.startDate.toLocaleDateString("en-IN")}`,
          350,
          yPos + 15,
        );
      doc.text(
        `To: ${invoiceData.endDate.toLocaleDateString("en-IN")}`,
        350,
        yPos + 30,
      );
