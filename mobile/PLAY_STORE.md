...
- **This guide is Android-only.** iOS has since been submitted to App Review
  (build 1.0 (2), rejected under guideline 5.1.1(v)); `ios/` remains gitignored
  and generated by prebuild. For the iOS build, submit and resubmission steps see
  [`docs/APP_STORE_RESUBMISSION.md`](../docs/APP_STORE_RESUBMISSION.md). Note that
  `submit.production` in `eas.json` still configures Android only — iOS submit
